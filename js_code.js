...
function mainFunc(word) {
	var rgx = /^[абвгдеёжзийклмнопрстуфхцчшэюяўқғҳъ]+$/;
	var word = autoCorrect(word).trim();
	var text = word.replace(/[аоуэияёюеў]/g,"V");
	var text = text.replace(/[бвгджзйклмнпрстфхцчшқғҳ]/g,"C");
	var parts = text.split("ъ");
	var map = createMap(parts);
	var rem = word;
